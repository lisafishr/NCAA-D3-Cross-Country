...
inputPanel(
  ...
  textInput("text2", label = h3("Search"), value = NULL , placeholder = "Eg. St. Olaf")
)
...
Nats17ind %>%
  ...
  filter(NAME %in% NAME[str_detect(.$NAME, input$text2)] | 
         PL %in% PL[str_detect(.$PL, input$text2)] | 
         Team %in% Team[str_detect(.$Team, input$text2)] | 
         Region %in% Region[str_detect(.$Region, input$text2)] ) %>%
